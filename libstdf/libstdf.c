...
{
	stdf_file *stdfParser = (stdf_file*)data;
	...
	stdfParser->fd_zip = NULL;

	if (__stdf_open_reg(data, flags, mode) == -1)
		return -1;

	d = zzip_dir_fdopen(stdfParser->fd, &err);
	if (d == NULL)
		return -1;
	de = zzip_readdir(d);
	if (de == NULL) {
	...
	stdfParser->fd_zip = zzip_file_open(d, de->d_name, O_RDONLY);
	zzip_dir_close(d);
	if (stdfParser->fd_zip == NULL)
		return -1;
...
{
	stdf_file *stdfParser = (stdf_file*)data;
	return (stdfParser->fd_zip==NULL) ? -1 : zzip_close(stdfParser->fd_zip);
}
...
{
	stdf_file *stdfParser = (stdf_file*)data;
	stdfParser->fd_gzip = NULL;

	if (__stdf_open_reg(data, flags, mode) == -1)
		return -1;

	stdfParser->fd_gzip = gzdopen(stdfParser->fd, "rb");
	if (stdfParser->fd_gzip == NULL)
		return -1;

	return stdfParser->fd;
}
...
{
	stdf_file *stdfParser = (stdf_file*)data;
	stdfParser->fd_bzip2 = NULL;

	if (__stdf_open_reg(data, flags, mode) == -1)
		return -1;

	stdfParser->fd_bzip2 = BZ2_bzdopen(stdfParser->fd, "rb");
	if (stdfParser->fd_bzip2 == NULL)
		return -1;
...
static int __stdf_close_bzip2(void *data)
{
	stdf_file *stdfParser = (stdf_file*)data;
	if (stdfParser->fd_bzip2 != NULL)
		BZ2_bzclose(stdfParser->fd_bzip2);
	return __stdf_close_reg(data);
}
